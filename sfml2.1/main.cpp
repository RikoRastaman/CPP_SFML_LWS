...
int main()
{
    ...
    sf::RenderWindow window(sf::VideoMode({WINDOW_WIDTH, WINDOW_HEIGHT}), "Wave motion of the ball.");
    sf::Clock clock;
    ...
    while (window.isOpen())
    {
        ...
        constexpr float amplitudeY = 200.f;
        ...
        const float x = x + speedX * dtime;
        const float y = amplitudeY * std::sin(wavePhase / periodY);
        const sf::Vector2f offset = {x, y};

        if ((x + 2 * BALL_SIZE >= WINDOW_HEIGHT) && (speedX > 0))
        {
            speedX = -speedX;
        }
        if ((x < 0) && (speedX < 0))
